...
public class KafkaSubmissionConsumerService {
    ...
    public void listen(String message) {
        ...
        try {
            ...
            if (submissionId == null || token == null) {
                System.out.println("Submission ID or Token is missing.");
                return;
            }// added this
    ...
    private void processJudge0Response(Long submissionId, String response) {
        try {
            if (response == null) {
                System.out.println("Received null response from Judge0 API.");
                return;
            }// added this error handling
            JsonNode rootNode = objectMapper.readTree(response);
            String status = rootNode.path("status").path("description").asText();
            String output = rootNode.path("stdout").asText();
            Double executionTime = rootNode.path("time").asDouble();
            Double memory = rootNode.path("memory").asDouble();
            if (output == null) {
                System.out.println("Output is null for submission ID: " + submissionId);
                output = ""; // or handle appropriately
            }
